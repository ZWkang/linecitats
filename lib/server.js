...
linecitats.prototype.init = function() {
  if (isNaN(this.port) || Number(this.port) !== Number(this.port)) {
    Reporter.error("port should be number");
  ...
  getPort({
    port: getPort.makeRange(this.port, this.port + 1000)
  })
    .then(port => {
      this._server = app.listen(port, () => {
        Reporter.info("starting in port " + port);
      });
    })
    .catch(reason => {
      Reporter.error(reason.message);
    });
};
